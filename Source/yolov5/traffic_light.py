...
ser.flushInput() #시리얼 USB포트를 통해 아두이노 보드가 연결되고, 해당 정보를 flush에 입력한다.

matplotlib.use('Agg') #비대화형 백엔드 AGG 설정: 래스터 그래픽 - Anti-Grain Geometry 엔진을 사용하는 고품질 이미지(파일형식은 png)
...
def laser_listener():
    ...
    sub = rospy.Subscriber('/YOLO', String, traffic_light)

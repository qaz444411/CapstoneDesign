...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + self.start_epoch):
            ...
            with torch.set_grad_enabled(False):
                for local_batch, (centers, lefts, rights) in enumerate(self.validationloader):
                    ...
                    for data in datas:
                        ...
                        fi = valid_loss / (local_batch + 1)
                    if local_batch % 100 == 0:
                        print("Validation Loss: {}".format(fi))

            
            # Save model
            # if epoch % 5 == 0 or epoch == self.epochs + self.start_epoch - 1:
            print("==> Save checkpoint ...")
            self.del_checkpoint(epoch)
            ...
            if fi < best_fitness and count < patience:
                self.del_bestpoint(best_epoch)
    ...
    def del_checkpoint(self, epoch):
        """Delete checkpoint."""
        try:
            os.remove(self.ckptroot + 'model-{}.h5'.format(epoch))
        except:
            pass

    def del_bestpoint(self, epoch):
        """Delete bestpoint."""
        try:
            os.remove(self.ckptroot + 'best_model-{}.h5'.format(epoch + 1))
        except:
            pass
    ...
    def save_bestpoint(self, state):
        ...
        torch.save(state, self.ckptroot + 'best_model-{}.h5'.format(state['epoch']))
        torch.save(state, self.ckptroot + 'model-{}.h5'.format(state['epoch']))
